...
// Dependencies
var pgPool = require('./connection-pool');
...
module.exports = (function() {
  ...
  var adapter = {
    ...
    teardown: function (conn, cb) {
      ...
      if (conn === null) {
        connections = {};
        // `pg@8` pools keep their idle clients (and therefore the event loop)
        // alive, so they have to be closed explicitly or `sails.lower()` hangs.
        return pgPool.endAll(cb);
      }
      if(!connections[conn]) return cb();
      delete connections[conn];
      if(!Object.keys(connections).length) return pgPool.endAll(cb);
      cb();
    },
    ...
    describe: function(connectionName, table, cb) {
      spawnConnection(connectionName, function __DESCRIBE__(client, cb) {
        ...
        var query = "SELECT x.nspname || '.' || x.relname as \"Table\", x.attnum as \"#\", x.attname as \"Column\", x.\"Type\"," +
          ...
          "pg_catalog.pg_get_expr(d.adbin, d.adrelid) as \"Default\" FROM (" +
                  ...
                  client.query(qs, q.values, function __FIND__(err, result) {
                    ...
                    result.rows.forEach(function(row) {
                      ...
                      if(!Array.isArray(q.instructions)) {
                        ...
                        // (`_.uniq` lost its iteratee argument in lodash 4 -- `_.uniqBy` is the replacement)
                        groupedRecords[row[fk]] = _.uniqBy(groupedRecords[row[fk]], q.instructions[1].childKey);
                      }
                    });
    ...
    stream: function(connectionName, table, options, stream) {
      ...
      // NOTE: upstream 0.11.x built the query with `new Query(...)`, an
      // identifier that does not exist anywhere in this file -- so `.stream()`
      // threw `ReferenceError: Query is not defined` on the very first call.
      // It is built with `Sequel` here, exactly like `find()` does.
      var _schema = connectionObject.schema;
      var processor = new Processor(_schema);
      ...
      // NOTE: `pg@8` removed the row-by-row `query.on('row')` event this method
      // used to rely on (a real row stream now needs the separate
      // `pg-query-stream` package). Konga never calls `.stream()`, so rather
      // than pull in another dependency the rows are fetched and then pushed
      // into the waterline stream one at a time.
      pgPool.connect(connectionObject.config, function(err, client, done) {
        ...
        client.query(_query.query[0], _query.values[0], function(err, result) {

          done(); // release the client back to the pool

          if (err) return stream.end(handleQueryError(err));

          result.rows.forEach(function(row) {
            stream.write(processor.cast(tableName, row));
          });
  ...
  function spawnConnection(connectionName, logic, cb) {
    ...
    // The `url` (if any) and the discrete host/port/user/... properties are
    // reconciled inside `./connection-pool`, which also translates `poolSize`
    // and `ssl` into their `pg@8` equivalents.
    var connectionConfig = connectionObject.config;

    // Grab a client instance from the client pool
    pgPool.connect(connectionConfig, function(err, client, done) {
